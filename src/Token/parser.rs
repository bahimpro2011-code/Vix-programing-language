...
impl Parser {
    ...
    fn parse_base_type(&mut self) -> Type {
        ...
        let base_type = match current {
            ...
            Token::Option => {
                ...
                let use_bracket = self.current() == Token::LeftBracket;
                ...
                if use_bracket {
                    self.expect(Token::RightBracket, vec![Token::Comma, Token::RightParen]);
                        } else {
                    self.expect(Token::RightParen, vec![Token::Comma]);
                }
            ...
            Token::Result => {
                ...
                Type::result(self.parse_type(), self.parse_type())
            }
    ...
    fn parse_primary(&mut self) -> Expr {
        ...
        let expr = match current {
            ...
            Token::Selfish => {self.advance();Expr::Var("self".to_string())}
            ...
            Token::Identifier(name) if matches!(name.as_str(), "array" | "slots" | "lists" | "char" | "panic") => {
            ...
            Token::Identifier(name) => {
                ...
                if self.current() == Token::Dot {
                    ...
                    if let Token::Identifier(member) = self.current() {
                        ...
                        let method_expr = self.parse_method_call(obj, method_name);
    ...
    fn parse_stmt(&mut self) -> Stmt {
        ...
        Token::Identifier(name) if self.peek(1) == Token::Colon => {
            ...
            Token::While => {
                ...
                Stmt::While(self.parse_expr(), body)
            }
            ...
            Token::For => {
                ...
                Stmt::For(var_name, self.parse_expr(), body)
            }
            _ => {
                let expr = self.parse_expr();
    ...
    fn parse_enum(&mut self) -> EnumDef {
        ...
        EnumDef { name, variants, is_public: true }
    }
    ...
    fn stmt_calls(
        ...
        for stmt in stmts {
            match stmt {
                ...
                Stmt::ModuleCall(module, func, args) => {
