...
impl Codegen {
    ...
    pub fn codegen_stmt(&mut self, stmt: &Stmt, body: &mut String) -> Result<(), ()> {
        ...
        match stmt {
            Stmt::TypedDeclaration { name, ty, value, .. } => {
                self.codegen_typed_declaration(name, ty, value, body, loc)
            }
    ...
    pub fn codegen_expr(&mut self, expr: &Expr, body: &mut String) -> Result<(String, Type), ()> {
        ...
        match expr {
            ...
            Expr::ResultOk(value) => self.codegen_result_ok(value, body),
            Expr::ResultErr(value) => self.codegen_result_err(value, body),
    ...
    pub fn codegen_program_full(
        ...
        for impl_block in impls {
            if let Err(_) = self.codegen_impl_block(impl_block) {
