...
impl Clang {
    ...
    fn create_cfg_stub() -> Result<PathBuf, String> {
        // i hope this works now bc i hated my life
        let stub_c = r#"
...
"#;
        ...
        let output = cmd.output().map_err(|e| format!("Failed to compile CFG stub: {}", e))?;
        ...
        if !output.status.success() {
            return Err(format!("CFG stub compilation failed"));
        }
        
        Ok(PathBuf::from("cfg_stub.obj"))
    }
    ...
    pub fn link_executable(
        ...
        println!("   {} Executable linked: {}", "✓".green(), exe_path);
        Ok(())
    }
