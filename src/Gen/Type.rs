...
impl TypeRegistry {
    ...
    pub fn generate_type_definition(&mut self, ty: &Type, arch: &ArchConfig) -> Option<String> {
        match ty {
            Type::Option { inner } => {
                ...
                let inner_c = inner.to_c_type(arch);
            ...
            Type::Result { ok, err } => {
                ...
                let ok_c = ok.to_c_type(arch);
                let err_c = err.to_c_type(arch);
                let ok_size = ok.size_bits(arch);
                let err_size = err.size_bits(arch);
                let _max_size = ok_size.max(err_size);
                let def = format!("typedef struct {{\n    uint8_t tag;\n    union {{\n        {} ok;\n        {} err;\n    }} data;\n}} {};\n",ok_c, err_c, type_id);
    ...
    fn get_option_type_id(&self, inner: &Type) -> String {
        format!("Option_{}", self.sanitize_type_name(&inner.name()))
    }
    
    fn get_result_type_id(&self, ok: &Type, err: &Type) -> String {
        format!(
            "Result_{}_{}",
            self.sanitize_type_name(&ok.name()),
            self.sanitize_type_name(&err.name())
        )
    }
    
    fn get_union_type_id(&self, variants: &[Type]) -> String {
        let names: Vec<String> = variants.iter().map(|v| self.sanitize_type_name(&v.name())).collect();
        format!("Union_{}", names.join("_"))
    }
    
    fn get_tuple_type_id(&self, fields: &[Type]) -> String {
        let names: Vec<String> = fields.iter().map(|f| self.sanitize_type_name(&f.name())).collect();
